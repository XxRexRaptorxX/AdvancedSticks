...
public class Config {
    ...
    static {
        ...
        HIDE_UPGRADE_SLOTS = CLIENT_BUILDER.comment("If you activate this, the upgrade slots are only displayed in the tooltip when you press 'SHIFT'").define("hide_upgrade_slots",
                false);
        CLIENT_BUILDER.pop();
    ...
    static {
        ...
        HANDLE_MATERIALS = SERVER_BUILDER.comment("Enabled handle materials").defineListAllowEmpty("handle_materials", Arrays.asList(ModItems.HANDLE_MATERIALS),
                () -> "material name", obj -> obj instanceof String string && isValidEntry(string));
        TOOL_HEAD_MATERIALS = SERVER_BUILDER.comment("Enabled tool head materials").defineListAllowEmpty("tool_head_materials", Arrays.asList(ModItems.TOOL_HEAD_MATERIALS),
                () -> "material name", obj -> obj instanceof String string && isValidEntry(string));
        DISABLED_MATERIAL_EFFECTS = SERVER_BUILDER.comment("List of materials where the effect should be disabled").defineListAllowEmpty("disabled_material_effects", List.of(),
                () -> "id:effect", obj -> obj instanceof String string && isValidEffect(string));;
        SERVER_BUILDER.pop();
    ...
    public static boolean getShowStickType() {
        return SHOW_STICK_TYPE.get();
    }


    public static boolean getShowMaterialStats() {
        return SHOW_MATERIAL_STATS.get();
    }


    public static boolean getJEIDescription() {
        return JEI_DESCRIPTION.get();
    }


    public static boolean getHideUpgradeSlots() {
        return HIDE_UPGRADE_SLOTS.get();
    }


    public static boolean getForceAllMaterials() {
        return FORCE_ALL_MATERIALS.get();
    }
    ...
    public static List<String> getHandleMaterials() {
        return (List<String>) HANDLE_MATERIALS.get();
    }


    public static List<String> getToolHeadMaterials() {
        return (List<String>) TOOL_HEAD_MATERIALS.get();
    }


    public static List<String> getDisabledMaterialEffects() {
        return (List<String>) DISABLED_MATERIAL_EFFECTS.get();
    }
