...
public class RecipeGen extends RecipeProvider {
        ...
        for (String handle : ModItems.HANDLE_MATERIALS) {
            ...
            for (String tool : ModItems.TOOL_TYPES) {
                if (!handle.equals("wood")) {
                    ...
                    netheriteSmithing(BuiltInRegistries.ITEM.getValue(getItemLoc(handle + "_stick_diamond_" + tool)),
                            BuiltInRegistries.ITEM.getValue(getItemLoc(handle + "_stick_netherite_" + tool)), handle);
                }
            }
    ...
    public void swordRecipe(TagKey<Item> handleTag, TagKey<Item> headTag, String headMaterial, Item result) {
        ...
        shaped(RecipeCategory.TOOLS, result).define('#', handleTag).define('X', headTag).pattern("X").pattern("X").pattern("#")
                .unlockedBy("has_" + headTag.location().getPath(), has(headTag)).save(output.withConditions(createTagNotEmptyConditions(headTag, headMaterial, handleTag)));
    }
    ...
    public void pickaxeRecipe(TagKey<Item> handleTag, TagKey<Item> headTag, String headMaterial, Item result) {
        ...
        shaped(RecipeCategory.TOOLS, result).define('#', handleTag).define('X', headTag).pattern("XXX").pattern(" # ").pattern(" # ")
                .unlockedBy("has_" + headTag.location().getPath(), has(headTag)).save(output.withConditions(createTagNotEmptyConditions(headTag, headMaterial, handleTag)));
    }
    ...
    public void axeRecipe(TagKey<Item> handleTag, TagKey<Item> headTag, String headMaterial, Item result) {
        ...
        shaped(RecipeCategory.TOOLS, result).define('#', handleTag).define('X', headTag).pattern("XX").pattern("#X").pattern("# ")
                .unlockedBy("has_" + headTag.location().getPath(), has(headTag)).save(output.withConditions(createTagNotEmptyConditions(headTag, headMaterial, handleTag)));
    }
    ...
    public void shovelRecipe(TagKey<Item> handleTag, TagKey<Item> headTag, String headMaterial, Item result) {
        ...
        shaped(RecipeCategory.TOOLS, result).define('#', handleTag).define('X', headTag).pattern("X").pattern("#").pattern("#")
                .unlockedBy("has_" + headTag.location().getPath(), has(headTag)).save(output.withConditions(createTagNotEmptyConditions(headTag, headMaterial, handleTag)));
    }
    ...
    public void hoeRecipe(TagKey<Item> handleTag, TagKey<Item> headTag, String headMaterial, Item result) {
        ...
        shaped(RecipeCategory.TOOLS, result).define('#', handleTag).define('X', headTag).pattern("XX").pattern("# ").pattern("# ")
                .unlockedBy("has_" + headTag.location().getPath(), has(headTag)).save(output.withConditions(createTagNotEmptyConditions(headTag, headMaterial, handleTag)));
    }
    ...
    public void upgradeRecipe(UpgradeItem result) {
        ...
        shapeless(RecipeCategory.MISC, result).group(References.MODID + "_upgrades").requires(ModItems.EMPTY_UPGRADE).requires(craftingTag)
    ...
    public void toolRecyclingRecipes() {
        for (String handle : ModItems.HANDLE_MATERIALS) {
            for (String tool : ModItems.TOOL_TYPES) {
                ...
                SimpleCookingRecipeBuilder.smelting(Ingredient.of(result), RecipeCategory.MISC, Items.IRON_NUGGET, 0.1F, 200).unlockedBy(getHasName(result), has(result))
                        .save(output, References.MODID + ":" + "smelting/" + handle + "_stick_iron_" + tool);

                SimpleCookingRecipeBuilder.blasting(Ingredient.of(result), RecipeCategory.MISC, Items.IRON_NUGGET, 0.1F, 100).unlockedBy(getHasName(result), has(result))
                        .save(output, References.MODID + ":" + "blasting/" + handle + "_stick_iron_" + tool);

                result = BuiltInRegistries.ITEM.getValue(ItemHelper.getLocation(References.MODID, handle + "_stick_gold_" + tool));
                AdvancedTools.LOGGER.info("Generate smelting recipes for " + getItemName(result));

                SimpleCookingRecipeBuilder.smelting(Ingredient.of(result), RecipeCategory.MISC, Items.GOLD_NUGGET, 0.1F, 200).unlockedBy(getHasName(result), has(result))
                        .save(output, References.MODID + ":" + "smelting/" + handle + "_stick_gold_" + tool);

                SimpleCookingRecipeBuilder.blasting(Ingredient.of(result), RecipeCategory.MISC, Items.GOLD_NUGGET, 0.1F, 100).unlockedBy(getHasName(result), has(result))
                        .save(output, References.MODID + ":" + "blasting/" + handle + "_stick_gold_" + tool);
            }
        }
    }
    ...
    private void netheriteSmithing(Item item, Item result, String conditionalMaterial) {
        ...
        SmithingTransformRecipeBuilder
                .smithing(Ingredient.of(Items.NETHERITE_UPGRADE_SMITHING_TEMPLATE), Ingredient.of(item), Ingredient.of(Items.NETHERITE_INGOT), RecipeCategory.COMBAT, result)
                .unlocks("has_netherite_ingot", has(Items.NETHERITE_INGOT))
                .save(output.withConditions(
                        new AndCondition(List.of(new NotCondition(new TagEmptyCondition<>(rodTag)), new NotCondition(new TagEmptyCondition<>(craftingMaterialTag))))),
                        References.MODID + ":smithing/" + getItemName(result));
    }
